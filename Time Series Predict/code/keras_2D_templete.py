...
def sliding_windows(data,window_size=1):
    ...
    for i in range(num_windows):
        ...
        temp_y = data.iloc[i+window_size,-1].reshape(-1,1)
        x_norm.append(scaler_x.transform(temp_x))   # 输入变量为除最后一列的数值
        y_norm.append(scaler_y.transform(temp_y))   # 输出变量为最后一列的数值
        y.append(temp_y)                            #  y
...
def data_split(data, split_rate=[0.7, 0.5],dim_trans = False):
    norm_data = {}
    x_norm = data['x_norm']
    y_norm = data['y_norm']
    y = data['y']
    ...
    if dim_trans == True:                     # n*s*m  s-滑动窗大小,n,s,m]
        # train data
        train_index = index_names[:train_num]
        x_train_norm = x_norm[:train_num, :]
        y_train_norm = y_norm[:train_num]
        y_train = y[:train_num]

        # valid data
        valid_index = index_names[train_num:train_num + valid_num]
        x_valid_norm = x_norm[train_num:train_num + valid_num, :]
        y_valid_norm = y_norm[train_num:train_num + valid_num, :]
        y_valid = y[train_num:train_num + valid_num, :]

        # test data
        test_index = index_names[train_num + valid_num:]
        x_test_norm = x_norm[train_num + valid_num:, :]
        y_test_norm = y_norm[train_num + valid_num:, :]
        y_test = y[train_num + valid_num:, :]

    else:                           # n*1*(s*m)  s-滑动窗大小[n,1,s*m]
        # train data
        train_index = index_names[:train_num]
        x_train_norm = x_norm[:train_num, :].reshape(-1, x_norm.shape[1] * x_norm.shape[2])
        y_train_norm = y_norm[:train_num].reshape(-1, 1)
        y_train = y[:train_num].reshape(-1, 1)

        # valid data
        valid_index = index_names[train_num:train_num + valid_num]
        x_valid_norm = x_norm[train_num:train_num + valid_num, :].reshape(-1,x_norm.shape[1] * x_norm.shape[2])
        y_valid_norm = y_norm[train_num:train_num + valid_num, :].reshape(-1, 1)
        y_valid = y[train_num:train_num + valid_num, :].reshape(-1, 1)

        # test data
        test_index = index_names[train_num + valid_num:]
        x_test_norm = x_norm[train_num + valid_num:, :].reshape(-1, x_norm.shape[1] * x_norm.shape[2])
        y_test_norm = y_norm[train_num + valid_num:, :].reshape(-1, 1)
        y_test = y[train_num + valid_num:, :].reshape(-1, 1)
...
def get_model():
    model = Sequential()
    model.add(Dense(units=128,activation='tanh'))#,input_shape=(x_dim,)
...
if __name__ == '__main__':
    ...
    dim_trans = False          # 是否变换数据，1 *（s*n），s*n，MLP只能是1
    ...
    # 3. 数据集划分
    train_data = data_split(norm_data, split_rate, dim_trans)  # 70%:15%:15%
    # 4. 获取数据
